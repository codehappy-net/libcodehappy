...
extern char* WgetFile(const char *url, bool timeout, bool silent, bool again, u32* flen = nullptr);

/*** As above, with cookies support. ***/
extern char* WgetFileCookies(const char *url, bool timeout, bool silent, char *cookies, bool again, u32* flen = nullptr);
...
extern RamFile* FetchURI(const char* URI);
extern RamFile* FetchURI(const std::string& URI);

/* Fetch a URI, and then write it to the specified filename. Returns true iff success. */
extern bool FetchURIToFile(const char* URI, const char* out_path);
extern bool FetchURIToFile(const std::string& URI, const std::string& out_path);
