...
void ggml_tallocr_alloc(struct ggml_tallocr * talloc, struct ggml_tensor * tensor) {
    ...
    if (talloc->offset + size > ggml_backend_buffer_get_size(talloc->buffer)) {
        L_FPRINTF(stderr, "%s: not enough space in the buffer to allocate %s (needed %zu, available %zu)\n",
...
static void remove_allocated_tensor(struct ggml_dyn_tallocr * alloc, size_t offset, const struct ggml_tensor * tensor) {
    ...
    L_FPRINTF(stderr, "tried to free tensor %s not found\n", tensor->name);
...
static size_t ggml_dyn_tallocr_alloc(struct ggml_dyn_tallocr * alloc, size_t size, const struct ggml_tensor * tensor) {
    ...
    if (best_fit_block == -1) {
        ...
        if (block->size >= size) {
            ...
            L_FPRINTF(stderr, "%s: not enough space in the buffer to allocate %zu bytes, largest block available %zu bytes\n",
    ...
    if (cur_max > alloc->max_size) {
        ...
        L_FPRINTF(stderr, "max_size = %.2f MB: tensors: ", cur_max / 1024.0 / 1024.0);
        for (int i = 0; i < 1024; i++) {
            if (alloc->allocated_tensors[i].tensor) {
                L_FPRINTF(stderr, "%s [%zx-%zx] (%.2f MB) ", alloc->allocated_tensors[i].tensor->name,
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: reallocating %s buffer from size %.02f MiB to %.02f MiB\n", __func__, ggml_backend_buft_name(galloc->bufts[i]), cur_size / 1024.0 / 1024.0, new_size / 1024.0 / 1024.0);
#endif
            ggml_backend_buffer_free(galloc->buffers[i]);
            galloc->buffers[i] = ggml_backend_buft_alloc_buffer(galloc->bufts[i], new_size);
            if (galloc->buffers[i] == NULL) {
                L_FPRINTF(stderr, "%s: failed to allocate %s buffer of size %zu\n", __func__, ggml_backend_buft_name(galloc->bufts[i]), new_size);
                return false;
            }
        }
    }
...
static bool ggml_gallocr_needs_realloc(ggml_gallocr_t galloc, struct ggml_cgraph * graph) {
    if (galloc->n_nodes != graph->n_nodes) {
#ifndef NDEBUG
        L_FPRINTF(stderr, "%s: graph has different number of nodes\n", __func__);
#endif
        return true;
    }
    ...
    if (galloc->n_leafs != graph->n_leafs) {
#ifndef NDEBUG
        L_FPRINTF(stderr, "%s: graph has different number of leafs\n", __func__);
#endif
        return true;
    }
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: node %s is not valid\n", __func__, node->name);
#endif
            return true;
        }
...
#ifndef NDEBUG
                L_FPRINTF(stderr, "%s: src %d (%s) of node %s is not valid\n", __func__, j, src->name, node->name);
#endif
                return true;
            }
        }
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: reallocating buffers automatically\n", __func__);
#endif
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: cannot reallocate multi buffer graph automatically, call reserve\n", __func__);
#endif
            return false;
        }
    }
...
ggml_backend_buffer_t ggml_backend_alloc_ctx_tensors_from_buft(struct ggml_context * ctx, ggml_backend_buffer_type_t buft) {
    ...
    for (struct ggml_tensor * t = first; t != NULL; t = ggml_get_next_tensor(ctx, t)) {
        ...
        if (this_size > max_size) {
            L_FPRINTF(stderr, "%s: tensor %s is too large to fit in a %s buffer (tensor size: %zu, max buffer size: %zu)\n",
                    __func__, t->name,
    ...
    if (n_buffers == 0) {
#ifndef NDEBUG
        L_FPRINTF(stderr, "%s: all tensors in the context are already allocated\n", __func__);
#endif
        return NULL;
    }
