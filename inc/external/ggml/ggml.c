...
static void set_numa_thread_affinity(int thread_n, int n_threads) {
    ...
    if (rv) {
            L_FPRINTF(stderr, "warning: pthread_setaffinity_np() failed: %s\n",
                    strerror(rv));
    }
...
static void ggml_graph_export_leaf(const struct ggml_tensor * tensor, FILE * fout) {
    ...
    L_FPRINTF(fout, "%-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
...
static void ggml_graph_export_node(const struct ggml_tensor * tensor, const char * arg, FILE * fout) {
    ...
    L_FPRINTF(fout, "%-6s %-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
            arg,
            ggml_type_name(tensor->type),
            ggml_op_name  (tensor->op),
            tensor->n_dims,
...
void ggml_graph_export(const struct ggml_cgraph * cgraph, const char * fname) {
    ...
    // print
    {
        ...
        L_FPRINTF(fout, "\n");
        L_FPRINTF(fout, "%-16s %8x\n", "magic",        GGML_FILE_MAGIC);
        L_FPRINTF(fout, "%-16s %8d\n", "version",      GGML_FILE_VERSION);
        L_FPRINTF(fout, "%-16s %8d\n", "leafs",        cgraph->n_leafs);
        L_FPRINTF(fout, "%-16s %8d\n", "nodes",        cgraph->n_nodes);
        L_FPRINTF(fout, "%-16s %" PRIu64 "\n", "eval", size_eval);

        // header
        L_FPRINTF(fout, "\n");
        L_FPRINTF(fout, "%-6s %-12s %8s %8s %8s %8s %8s %16s %16s %16s %16s %16s %16s\n",
        ...
        // header
        L_FPRINTF(fout, "\n");
        L_FPRINTF(fout, "%-6s %-6s %-12s %8s %8s %8s %8s %8s %16s %16s %16s %16s %8s %16s %16s\n",
    ...
    // write binary data
    {
        ...
        if (!fout) {
            L_FPRINTF(stderr, "%s: failed to open %s\n", __func__, fname);
            return;
        }
                ...
                // output the op arguments
                {
                    ...
                    for (int j = 0; j < GGML_MAX_SRC; ++j) {
                        if (args[j]) {
                            ...
                            if (idx == -1) {
                                L_FPRINTF(stderr, "%s: failed to find tensor, arg = %d, node = %d\n", __func__, j, i);
                                return;
                            }
...
struct ggml_cgraph ggml_graph_import(const char * fname, struct ggml_context ** ctx_data, struct ggml_context ** ctx_eval) {
    ...
    // read file into data
    {
        ...
        // create the data context
        {
            ...
            *ctx_data = ggml_init(params);
            ...
            if (!*ctx_data) {
                L_FPRINTF(stderr, "%s: failed to create ggml context\n", __func__);
                fclose(fin);
                return result;
            }
        }
...

        {
            ...
            if (ret != fsize) {
                L_FPRINTF(stderr, "%s: failed to read %s\n", __func__, fname);
                fclose(fin);
                return result;
            }
        }
    ...
    // populate result
    {
        ...
        if (magic != GGML_FILE_MAGIC) {
            L_FPRINTF(stderr, "%s: invalid magic number, got %08x\n", __func__, magic);
            return result;
        }
        ...
        if (version != GGML_FILE_VERSION) {
            L_FPRINTF(stderr, "%s: invalid version number\n", __func__);
            return result;
        }
        ...
        // leafs
        {
            ...
            for (uint32_t i = 0; i < n_leafs; ++i) {
                ...
                L_FPRINTF(stderr, "%s: loaded leaf %d: '%16s', %3d dims, %9zu bytes\n", __func__, i, tensor->name, n_dims, ggml_nbytes(tensor));
            }
        }
        ...
        // nodes
        {
            ...
            for (uint32_t i = 0; i < n_nodes; ++i) {
                ...
                L_FPRINTF(stderr, "%s: loaded node %d: '%16s', %3d dims, %9zu bytes\n", __func__, i, tensor->name, n_dims, ggml_nbytes(tensor));
            }
        }
    }
...
static void ggml_graph_dump_dot_node_edge(FILE * fp, const struct ggml_cgraph * gb, struct ggml_tensor * node, struct ggml_tensor * parent, const char * label)  {
    ...
    L_FPRINTF(fp, "  \"%p\":%s -> \"%p\":%s [ arrowhead = %s; style = %s; label = \"%s\"; ]\n",
...
void ggml_graph_dump_dot(const struct ggml_cgraph * gb, const struct ggml_cgraph * gf, const char * filename) {
    ...
    L_FPRINTF(fp, "digraph G {\n");
    L_FPRINTF(fp, "  newrank = true;\n");
    L_FPRINTF(fp, "  rankdir = LR;\n");
    ...
    for (int i = 0; i < gb->n_nodes; i++) {
        ...
        if (strlen(node->name) > 0) {
            L_FPRINTF(fp, "%s (%s)|", node->name, ggml_type_name(node->type));
        } else {
            L_FPRINTF(fp, "(%s)|", ggml_type_name(node->type));
        }

        if (node->n_dims == 2) {
            L_FPRINTF(fp, "%d [%" PRId64 ", %" PRId64 "] | <x>%s", i, node->ne[0], node->ne[1], ggml_op_symbol(node->op));
        } else {
            L_FPRINTF(fp, "%d [%" PRId64 ", %" PRId64 ", %" PRId64 "] | <x>%s", i, node->ne[0], node->ne[1], node->ne[2], ggml_op_symbol(node->op));
        }
    ...
    for (int i = 0; i < gb->n_leafs; i++) {
        ...
        L_FPRINTF(fp, "CONST %d [%" PRId64 ", %" PRId64 "]", i, node->ne[0], node->ne[1]);
        if (ggml_nelements(node) < 5) {
            L_FPRINTF(fp, " | (");
            for (int j = 0; j < ggml_nelements(node); j++) {
                if (node->type == GGML_TYPE_I8 || node->type == GGML_TYPE_I16 || node->type == GGML_TYPE_I32) {
                    L_FPRINTF(fp, "%d", ggml_get_i32_1d(node, j));
                }
                else if (node->type == GGML_TYPE_F32 || node->type == GGML_TYPE_F16) {
                    L_FPRINTF(fp, "%.1e", (double)ggml_get_f32_1d(node, j));
                }
                else {
                    L_FPRINTF(fp, "#");
                }
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    // check the magic before making allocations
    {
        ...
        if (magic != GGUF_MAGIC) {
            L_FPRINTF(stderr, "%s: invalid magic number %08x\n", __func__, magic);
            fclose(file);
            return NULL;
        }
    }
    ...
    // read the header
    {
        ...
        if (!ok) {
            L_FPRINTF(stderr, "%s: failed to read header\n", __func__);
            fclose(file);
            gguf_free(ctx);
            return NULL;
        }
    ...
    // read the kv pairs
    {
        ...
        for (uint32_t i = 0; i < ctx->header.n_kv; ++i) {
            ...
            //L_FPRINTF(stderr, "%s: reading kv %d\n", __func__, i);

            ok = ok && gguf_fread_str(file, &kv->key,                    &offset);
            ok = ok && gguf_fread_el (file, &kv->type, sizeof(kv->type), &offset);

            //L_FPRINTF(stderr, "%s: reading kv with key %s\n", __func__, kv->key.data);
        ...
        if (!ok) {
            L_FPRINTF(stderr, "%s: failed to read key-value pairs\n", __func__);
            fclose(file);
            gguf_free(ctx);
            return NULL;
        }
    ...
    // read the tensor infos
    {
        ...
        for (uint32_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (!ok) {
                L_FPRINTF(stderr, "%s: failed to read tensor info\n", __func__);
                fclose(file);
                gguf_free(ctx);
                return NULL;
            }
    ...
    if (params.ctx != NULL) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        if (!params.no_alloc) {
            ...
            if (!ok) {
                L_FPRINTF(stderr, "%s: failed to read tensor data\n", __func__);
                fclose(file);
                ggml_free(ctx_data);
                gguf_free(ctx);
                return NULL;
        ...
        if (!ok) {
            L_FPRINTF(stderr, "%s: failed to read the tensor data\n", __func__);
            fclose(file);
            ggml_free(ctx_data);
            gguf_free(ctx);
            return NULL;
