...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads, int n_cur_threads) {
    ...
    switch (node->op) {
        ...
        default:
            {
                L_FPRINTF(stderr, "%s: op not implemented: ", __func__);
    ...
    // populate result
    {
        ...
        // leafs
        {
            ...
            for (uint32_t i = 0; i < n_leafs; ++i) {
                ...
                L_FPRINTF(stderr, "%s: loaded leaf %u: '%16s', %9zu bytes\n", __func__, i, tensor->name, ggml_nbytes(tensor));
            }
        }
        ...
        // nodes
        {
            ...
            for (uint32_t i = 0; i < n_nodes; ++i) {
                ...
                L_FPRINTF(stderr, "%s: loaded node %u: '%16s', %9zu bytes\n", __func__, i, tensor->name, ggml_nbytes(tensor));
            }
        }
    }
...
static bool gguf_fread_str(FILE * file, struct gguf_str * p, size_t * offset) {
    ...
    if (p->n == SIZE_MAX) {
        L_FPRINTF(stderr, "%s: invalid string length (%" PRIu64 ")\n", __func__, p->n);
        return false;
    }
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    // check the magic before making allocations
    {
        ...
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                L_FPRINTF(stderr, "%s: invalid magic characters '%c%c%c%c'\n", __func__, magic[0], magic[1], magic[2], magic[3]);
                fclose(file);
                return NULL;
            }
        }
    ...
    // read the header
    {
        ...
        if (ctx->header.version == 1) {
            L_FPRINTF(stderr, "%s: GGUFv1 is no longer supported. please use a more up-to-date version\n", __func__);
            fclose(file);
            gguf_free(ctx);
            return NULL;
        }
                            ...
                            case GGUF_TYPE_BOOL:
                                {
                                    ...
                                    if (kv->value.arr.n >= SIZE_MAX/gguf_type_size(kv->value.arr.type)) {
                                        L_FPRINTF(stderr, "%s: array size is too large (%" PRIu64 ")\n", __func__, kv->value.arr.n);
                                        fclose(file);
                                        gguf_free(ctx);
                                        return NULL;
                                    }
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            for (uint64_t j = 0; j < i; ++j) {
                if (strcmp(info->name.data, ctx->infos[j].name.data) == 0) {
                    L_FPRINTF(stderr, "%s: duplicated tensor name %s\n", __func__, info->name.data);
                    ok = false;
                }
            }
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ne % ggml_blck_size(info->type) != 0) {
                L_FPRINTF(stderr, "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%d)\n",
