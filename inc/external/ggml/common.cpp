...
bool parse_kv_override(const char * data, std::vector<llama_model_kv_override> & overrides) {
    ...
    if (sep == nullptr || sep - data >= 128) {
        L_FPRINTF(stderr, "%s: malformed KV override '%s'\n", __func__, data);
        return false;
    }
    ...
    if (strncmp(sep, "int:", 4) == 0) {
        ...
        if (std::strcmp(sep, "true") == 0) {
            ...
            L_FPRINTF(stderr, "%s: invalid boolean value for KV override '%s'\n", __func__, data);
            return false;
        }
        ...
        if (strlen(sep) > 127) {
            L_FPRINTF(stderr, "%s: malformed KV override '%s', value cannot exceed 127 chars\n", __func__, data);
            return false;
        }
        strncpy(kvo.val_str, sep, 127);
        kvo.val_str[127] = '\0';
    } else {
        L_FPRINTF(stderr, "%s: invalid type for KV override '%s'\n", __func__, data);
        return false;
    }
...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "-bf" || arg == "--binary-file") {
        ...
        L_FPRINTF(stderr, "Read %zu bytes from binary file %s\n", params.prompt.size(), argv[i]);
        return true;
    }
    if (arg == "-f" || arg == "--file") {
        if (++i >= argc) {
    ...
    if (arg == "--gpu-layers-draft" || arg == "-ngld" || arg == "--n-gpu-layers-draft") {
        ...
        if (!llama_supports_gpu_offload()) {
            L_FPRINTF(stderr, "warning: not compiled with GPU offload support, --n-gpu-layers-draft option will be ignored\n");
            L_FPRINTF(stderr, "warning: see main README.md for information on enabling GPU BLAS support\n");
        }
        return true;
    }
    if (arg == "--main-gpu" || arg == "-mg") {
        ...
        L_FPRINTF(stderr, "warning: llama.cpp was compiled without CUDA/SYCL. Setting the main GPU has no effect.\n");
...
#ifdef GGML_USE_SYCL
            L_FPRINTF(stderr, "warning: The split mode value:[row] is not supported by llama.cpp with SYCL. It's developing.\nExit!\n");
            exit(1);
#endif // GGML_USE_SYCL
...
#ifndef GGML_USE_CUDA_SYCL
        L_FPRINTF(stderr, "warning: llama.cpp was compiled without CUDA/SYCL. Setting the split mode has no effect.\n");
#endif // GGML_USE_CUDA_SYCL
        return true;
    }
    if (arg == "--tensor-split" || arg == "-ts") {
        ...
        L_FPRINTF(stderr, "warning: llama.cpp was compiled without CUDA/SYCL/Vulkan. Setting a tensor split has no effect.\n");
    ...
    if (arg == "--version") {
        L_FPRINTF(stderr, "version: %d (%s)\n", LLAMA_BUILD_NUMBER, LLAMA_COMMIT);
        L_FPRINTF(stderr, "built with %s for %s\n", LLAMA_COMPILER, LLAMA_BUILD_TARGET);
        exit(0);
    }
    ...
    if (arg == "--override-kv") {
        ...
        if (!parse_kv_override(argv[i], params.kv_overrides)) {
            L_FPRINTF(stderr, "error: Invalid type for KV override: %s\n", argv[i]);
            invalid_param = true;
            return true;
        }
        return true;
...
static bool llama_download_file(const std::string & url, const std::string & path) {
    ...
    if (!curl) {
        L_FPRINTF(stderr, "%s: error initializing libcurl\n", __func__);
        return false;
    }
    ...
    if (file_exists) {
        ...
        if (metadata_in.good()) {
            try {
                ...
                L_FPRINTF(stderr, "%s: previous metadata file found %s: %s\n", __func__, metadata_path.c_str(), metadata.dump().c_str());
                if (metadata.contains("url") && metadata.at("url").is_string()) {
                    auto previous_url = metadata.at("url").get<std::string>();
                    if (previous_url != url) {
                        L_FPRINTF(stderr, "%s: Model URL mismatch: %s != %s\n", __func__, url.c_str(), previous_url.c_str());
                        return false;
                    }
                }
                ...
                L_FPRINTF(stderr, "%s: error reading metadata file %s: %s\n", __func__, metadata_path.c_str(), e.what());
                return false;
            }
        }
    } else {
        L_FPRINTF(stderr, "%s: no previous model file found %s\n", __func__, path.c_str());
    }
    ...
    llama_load_model_from_url_headers headers;
    {
        ...
        if (res != CURLE_OK) {
            L_FPRINTF(stderr, "%s: curl_easy_perform() failed: %s\n", __func__, curl_easy_strerror(res));
            return false;
        }
        ...
        if (http_code != 200) {
            ...
            force_download = true;
            L_FPRINTF(stderr, "%s: HEAD invalid http status code received: %ld\n", __func__, http_code);
        }
    }
    ...
    if (!should_download) {
        if (!etag.empty() && etag != headers.etag) {
            L_FPRINTF(stderr, "%s: ETag header is different (%s != %s): triggering a new download\n", __func__, etag.c_str(), headers.etag.c_str());
            should_download = true;
        } else if (!last_modified.empty() && last_modified != headers.last_modified) {
            L_FPRINTF(stderr, "%s: Last-Modified header is different (%s != %s): triggering a new download\n", __func__, last_modified.c_str(), headers.last_modified.c_str());
            should_download = true;
        }
    }
    if (should_download) {
        ...
        if (file_exists) {
            L_FPRINTF(stderr, "%s: deleting previous downloaded file: %s\n", __func__, path.c_str());
            if (remove(path.c_str()) != 0) {
                L_FPRINTF(stderr, "%s: unable to delete file: %s\n", __func__, path.c_str());
                return false;
            }
        }
        ...
        if (!outfile) {
            L_FPRINTF(stderr, "%s: error opening local file for writing: %s\n", __func__, path.c_str());
            return false;
        }
        ...
        // start the download
        L_FPRINTF(stderr, "%s: downloading from %s to %s (server_etag:%s, server_last_modified:%s)...\n", __func__,
        ...
        if (http_code < 200 || http_code >= 400) {
            L_FPRINTF(stderr, "%s: invalid http status code received: %ld\n", __func__, http_code);
            return false;
        }
        ...
        L_FPRINTF(stderr, "%s: file metadata saved: %s\n", __func__, metadata_path.c_str());

        if (rename(path_temporary.c_str(), path.c_str()) != 0) {
            L_FPRINTF(stderr, "%s: unable to rename file: %s to %s\n", __func__, path_temporary.c_str(), path.c_str());
            return false;
        }
    }
...
struct llama_model * llama_load_model_from_url(
    ...
    if (!model_url || strlen(model_url) == 0) {
        L_FPRINTF(stderr, "%s: invalid model_url\n", __func__);
        return NULL;
    }
...
struct llama_model * llama_load_model_from_url(
        ...
        const char * /*path_model*/,
        const struct llama_model_params & /*params*/) {
    L_FPRINTF(stderr, "%s: llama.cpp built without libcurl, downloading from an url not supported.\n", __func__);
    return nullptr;
}
...
struct llama_model * llama_load_model_from_hf(
        ...
        const char * /*path_model*/,
        const struct llama_model_params & /*params*/) {
    L_FPRINTF(stderr, "%s: llama.cpp built without libcurl, downloading from Hugging Face not supported.\n", __func__);
    return nullptr;
}
...
void dump_non_result_info_yaml(FILE * stream, const gpt_params & params, const llama_context * lctx,
    ...
    L_FPRINTF(stream, "cpu_has_avx_vnni: %s\n",    ggml_cpu_has_avx_vnni()    ? "true" : "false");
    ...
    L_FPRINTF(stream, "cpu_has_cuda: %s\n",        ggml_cpu_has_cuda()        ? "true" : "false");
    L_FPRINTF(stream, "cpu_has_vulkan: %s\n",      ggml_cpu_has_vulkan()      ? "true" : "false");
    L_FPRINTF(stream, "cpu_has_clblast: %s\n",     ggml_cpu_has_clblast()     ? "true" : "false");
    L_FPRINTF(stream, "cpu_has_kompute: %s\n",     ggml_cpu_has_kompute()     ? "true" : "false");
    ...
    L_FPRINTF(stream, "cpu_has_matmul_int8: %s\n", ggml_cpu_has_matmul_int8() ? "true" : "false");
    ...
    L_FPRINTF(stream, "interactive_specials: %s # default: false\n", params.interactive_specials ? "true" : "false");
    ...
    L_FPRINTF(stream, "lora_scaled:\n");
    for (std::tuple<std::string, float> la : params.lora_adapter) {
        if (std::get<1>(la) == 1.0f) {
            continue;
        }
        L_FPRINTF(stream, "  - %s: %f\n", std::get<0>(la).c_str(), std::get<1>(la));
    }
    L_FPRINTF(stream, "lora_base: %s\n", params.lora_base.c_str());
    L_FPRINTF(stream, "main_gpu: %d # default: 0\n", params.main_gpu);
    L_FPRINTF(stream, "min_keep: %d # default: 0 (disabled)\n", sparams.min_keep);
    ...
    L_FPRINTF(stream, "model: %s # default: %s\n", params.model.c_str(), DEFAULT_MODEL_PATH);
    ...
    L_FPRINTF(stream, "penalize_nl: %s # default: false\n", sparams.penalize_nl ? "true" : "false");
    ...
    L_FPRINTF(stream, "seed: %u # default: -1 (random seed)\n", params.seed);
    L_FPRINTF(stream, "simple_io: %s # default: false\n", params.simple_io ? "true" : "false");
    L_FPRINTF(stream, "cont_batching: %s # default: false\n", params.cont_batching ? "true" : "false");
    L_FPRINTF(stream, "flash_attn: %s # default: false\n", params.flash_attn ? "true" : "false");
...
static llama_control_vector_data llama_control_vector_load_one(const llama_control_vector_load_info & load_info) {
    ...
    // calculate size of ctx needed for tensors, ensure tensors are f32, and find max layer
    {
        ...
        if (!meta_ctx_gguf) {
            L_FPRINTF(stderr, "%s: failed to load control vector from %s\n", __func__, load_info.fname.c_str());
            ggml_free(meta_ctx);
            return result;
        }
        ...
        for (int i = 0; i < n_tensors; i++) {
            ...
            if (dotpos != std::string::npos && name.substr(0, dotpos) == "direction") {
                try {
                    ...
                    if (layer == 0) {
                        L_FPRINTF(stderr, "%s: direction tensor invalid in %s\n", __func__, load_info.fname.c_str());
                        ggml_free(meta_ctx);
            ...
            if (result.n_embd == -1) {
                ...
                L_FPRINTF(stderr, "%s: direction tensor sizes mismatched in %s\n", __func__, load_info.fname.c_str());
                ggml_free(meta_ctx);
                gguf_free(meta_ctx_gguf);
                return result;
            }
    ...
    if (n_tensors == 0) {
        L_FPRINTF(stderr, "%s: no direction tensors found in %s\n", __func__, load_info.fname.c_str());
        return result;
    }
...
llama_control_vector_data llama_control_vector_load(const std::vector<llama_control_vector_load_info> & load_infos) {
    ...
    for (const auto & info : load_infos) {
        ...
        if (result.n_embd != -1 && (result.n_embd != cur.n_embd || result.data.size() != cur.data.size())) {
            L_FPRINTF(stderr, "%s: control vector in %s does not match previous vector dimensions\n", __func__, info.fname.c_str());
            return result;
        }
    ...
    if (result.n_embd == -1) {
        L_FPRINTF(stderr, "%s: no vectors passed\n", __func__);
    }
