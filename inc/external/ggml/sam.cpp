...
#define SAM_FPRINTF(...)	fprintf(__VA_ARGS__)
#else
#define SAM_FPRINTF(...)
#endif
...
struct sam_hparams {
    ...
    std::vector<int32_t> global_attn_indices() const {
        switch (n_enc_state) {
            ...
            default:
                {
                    SAM_FPRINTF(stderr, "%s: unsupported n_enc_state = %d\n", __func__, n_enc_state);
                } break;
        };
...
bool sam_image_load_from_file(const std::string & fname, sam_image_u8 & img) {
    ...
    if (!data) {
        SAM_FPRINTF(stderr, "%s: failed to load '%s'\n", __func__, fname.c_str());
        return false;
    }
...
bool sam_model_load(const sam_params & params, sam_model & model) {
    SAM_FPRINTF(stderr, "%s: loading model from '%s' - please wait ...\n", __func__, params.model.c_str());

    auto fin = std::ifstream(params.model, std::ios::binary);
    if (!fin) {
        SAM_FPRINTF(stderr, "%s: failed to open '%s'\n", __func__, params.model.c_str());
        return false;
    }
    ...
    // verify magic
    {
        ...
        if (magic != 0x67676d6c) {
            SAM_FPRINTF(stderr, "%s: invalid model file '%s' (bad magic)\n", __func__, params.model.c_str());
            return false;
        }
    }
    ...
    if (wtype == GGML_TYPE_COUNT) {
        SAM_FPRINTF(stderr, "%s: invalid model file '%s' (bad ftype value %d)\n",
    ...
    const size_t ctx_size = [&]() {
        ...
        SAM_FPRINTF(stderr, "%s: ggml ctx size = %6.2f MB\n", __func__, ctx_size/(1024.0*1024.0));
    ...
    // create the ggml context
    {
        ...
        if (!ctx) {
            SAM_FPRINTF(stderr, "%s: ggml_init() failed\n", __func__);
            return false;
        }
    }
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            if (model.tensors.find(name.data()) == model.tensors.end()) {
                SAM_FPRINTF(stderr, "%s: unknown tensor '%s' in model file\n", __func__, name.data());
                return false;
            }
            ...
            if (ggml_nelements(tensor) != nelements) {
                SAM_FPRINTF(stderr, "%s: tensor '%s' has wrong size in model file: got %d, expected %d\n",
            ...
            if (tensor->ne[0] != ne[0] || tensor->ne[1] != ne[1] || tensor->ne[2] != ne[2] || tensor->ne[3] != ne[3]) {
                SAM_FPRINTF(stderr, "%s: tensor '%s' has wrong shape in model file: got [%d, %d, %d, %d], expected [%d, %d, %d, %d]\n",
                        __func__, name.data(),
            ...
            switch (ftype) {
                ...
                default:
                        {
                            SAM_FPRINTF(stderr, "%s: unknown ftype %d in model file\n", __func__, ftype);
                            return false;
                        }
            };

            if ((nelements*bpe)/ggml_blck_size(tensor->type) != ggml_nbytes(tensor)) {
                SAM_FPRINTF(stderr, "%s: tensor '%s' has wrong size in model file: got %zu, expected %zu\n",
        ...
        if (n_tensors != int(model.tensors.size())) {
            SAM_FPRINTF(stderr, "%s: model file has %d tensors, but %d tensors were expected\n", __func__, n_tensors, (int) model.tensors.size());
            return false;
        }

        SAM_FPRINTF(stderr, " done\n");

        SAM_FPRINTF(stderr, "%s: model size = %8.2f MB / num tensors = %d\n", __func__, total_size/1024.0/1024.0, n_tensors);
    }
...
bool sam_write_masks(const sam_hparams& hparams, int nx, int ny, const sam_state & state, bitmap_masks* bmps_out) {
	...
	bmps_out->masks[bmps_out->nmasks].y_min = min_iy;
	bmps_out->masks[bmps_out->nmasks].y_max = max_iy;
	bmps_out->masks[bmps_out->nmasks].iou = iou_data[i];
	bmps_out->masks[bmps_out->nmasks].stability_score = stability_score;
	SBitmap* bmp = bmps_out->masks[bmps_out->nmasks].bmp;
	int idx = 0;
	bmps_out->nmasks++;
	bmp->clear();
	for (int yy = 0; yy < bmps_out->ny; ++yy) {
		for (int xx = 0; xx < bmps_out->nx; ++xx) {
			if (res.data[idx] == 255)
				bmp->put_pixel(xx, yy, C_WHITE);
			++idx;
		}
	}
...
struct ggml_cgraph  * sam_build_fast_graph(
    ...
    if (!pe_img_dense) {
        SAM_FPRINTF(stderr, "%s: failed to get dense positional encoding\n", __func__);
        return {};
    }

    if (!sam_decode_mask(model, enc_res, pe_img_dense, ctx0, gf, state)) {
         SAM_FPRINTF(stderr, "%s: failed to decode mask\n", __func__);
         return {};
    }
...
void sbitmap_to_sam_image(const SBitmap* img_in, sam_image_u8* img_out) {
	img_out->nx = (int) img_in->width();
	img_out->ny = (int) img_in->height();
	for (int y = 0; y < img_out->ny; ++y) {
		for (int x = 0; x < img_out->nx; ++x) {
			RGBColor c = img_in->get_pixel(x, y);
			img_out->data.push_back((uint8_t) RGB_RED(c));
			img_out->data.push_back((uint8_t) RGB_GREEN(c));
			img_out->data.push_back((uint8_t) RGB_BLUE(c));
		}
	}
}
...
bitmap_masks* sam_mask_segment(const SBitmap* img_in, const std::string& img_path, const std::string& sam_model_path, float x, float y, int rng_seed) {
    ...
    sam_params params;
    params.model = sam_model_path;
    params.fname_inp = img_path;
    params.pt.x = x;
    params.pt.y = y;
    ...
    SAM_FPRINTF(stderr, "%s: seed = %d\n", __func__, params.seed);

    // load the image
    sam_image_u8 img0;
    if (img_in != nullptr) {
    	sbitmap_to_sam_image(img_in, &img0);
    } else if (!sam_image_load_from_file(params.fname_inp, img0)) {
        SAM_FPRINTF(stderr, "%s: failed to load image from '%s'\n", __func__, params.fname_inp.c_str());
        return nullptr;
    }
    SAM_FPRINTF(stderr, "%s: loaded image '%s' (%d x %d)\n", __func__, params.fname_inp.c_str(), img0.nx, img0.ny);

    ret = new bitmap_masks;
    ret->nx = img0.nx;
    ret->ny = img0.ny;
    ...
    if (!sam_image_preprocess(img0, img1)) {
        SAM_FPRINTF(stderr, "%s: failed to preprocess image\n", __func__);
        goto LErr2;
    }
    SAM_FPRINTF(stderr, "%s: preprocessed image (%d x %d)\n", __func__, img1.nx, img1.ny);
    ...
    // report timing
    {
        ...
        SAM_FPRINTF(stderr, "\n\n");
        SAM_FPRINTF(stderr, "%s:     load time = %8.2f ms\n", __func__, t_load_us/1000.0f);
