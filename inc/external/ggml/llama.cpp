...
struct llama_mmap {
    ...
    llama_mmap(struct llama_file * file, size_t prefetch = (size_t) -1 /* -1 = max value */, bool numa = false) {
        ...
        if (prefetch > 0) {
            ...
            if (posix_madvise(addr, std::min(file->size, prefetch), POSIX_MADV_WILLNEED)) {
                L_FPRINTF(stderr, "warning: posix_madvise(.., POSIX_MADV_WILLNEED) failed: %s\n",
                        strerror(errno));
            }
        }
        if (numa) {
            ...
            if (posix_madvise(addr, file->size, POSIX_MADV_RANDOM)) {
                L_FPRINTF(stderr, "warning: posix_madvise(.., POSIX_MADV_RANDOM) failed: %s\n",
                        strerror(errno));
            }
        }
    }
        ...
        if (prefetch) {
            ...
            if (pPrefetchVirtualMemory) {
                ...
                if (!pPrefetchVirtualMemory(GetCurrentProcess(), 1, &range, 0)) {
                    L_FPRINTF(stderr, "warning: PrefetchVirtualMemory failed: %s\n",
    ...
    ~llama_mmap() {
        if (!UnmapViewOfFile(addr)) {
            L_FPRINTF(stderr, "warning: UnmapViewOfFile failed: %s\n",
                    llama_format_win_err(GetLastError()).c_str());
        }
    }
#else
...
struct llama_mlock {
    ...
    bool raw_lock(const void * addr, size_t size) const {
        ...
        L_FPRINTF(stderr, "warning: failed to mlock %zu-byte buffer (after previously locking %zu bytes): %s\n%s",
    ...
    static void raw_unlock(void * addr, size_t size) {
        if (munlock(addr, size)) {
            L_FPRINTF(stderr, "warning: failed to munlock buffer: %s\n", std::strerror(errno));
        }
    }
#elif defined(_WIN32)
    static constexpr bool SUPPORTED = true;
    ...
    bool raw_lock(void * ptr, size_t len) const {
        for (int tries = 1; ; tries++) {
            ...
            if (tries == 2) {
                L_FPRINTF(stderr, "warning: failed to VirtualLock %zu-byte buffer (after previously locking %zu bytes): %s\n",
            ...
            if (!GetProcessWorkingSetSize(GetCurrentProcess(), &min_ws_size, &max_ws_size)) {
                L_FPRINTF(stderr, "warning: GetProcessWorkingSetSize failed: %s\n",
                        llama_format_win_err(GetLastError()).c_str());
                return false;
            }
            ...
            if (!SetProcessWorkingSetSize(GetCurrentProcess(), min_ws_size, max_ws_size)) {
                L_FPRINTF(stderr, "warning: SetProcessWorkingSetSize failed: %s\n",
                        llama_format_win_err(GetLastError()).c_str());
                return false;
            }
        }
    ...
    static void raw_unlock(void * ptr, size_t len) {
        if (!VirtualUnlock(ptr, len)) {
            L_FPRINTF(stderr, "warning: failed to VirtualUnlock buffer: %s\n",
                    llama_format_win_err(GetLastError()).c_str());
        }
    }
#else
    ...
    bool raw_lock(const void * addr, size_t len) const {
        L_FPRINTF(stderr, "warning: mlock not supported on this system\n");
        return false;
    }
        ...
        while (it != buffer.end()) {
            ...
            if (fragment.type == FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT) {
                ...
                while (true) {
                    ...
                    L_FPRINTF(stderr, "FF: (%ld %ld %ld) '%s'\n", raw_text->length(), raw_text_base_offset, raw_text_base_length, raw_text->substr(raw_text_base_offset, raw_text_base_length).c_str());
#endif
...
#ifdef PRETOKENIZERDEBUG
                        L_FPRINTF(stderr,"TT: (%ld %ld %ld) '%s'\n", raw_text.length(), fragment.offset, fragment.length, raw_text.c_str());
#endif
...
int llama_n_layer(const struct llama_model * model) {
...
static void llama_copy_state_data_internal(struct llama_context * ctx, llama_data_context * data_ctx) {
    ...
    // copy kv cache
    {
        ...
        if (kv_buf_size) {
            ...
            ggml_context * cpy_ctx = ggml_init({ 4096, NULL, /* no_alloc */ true, /* dynamic */ false });
            ggml_cgraph gf{};
...
void llama_dump_timing_info_yaml(FILE * stream, const llama_context * ctx) {
    L_FPRINTF(stream, "\n");
    L_FPRINTF(stream, "###########\n");
    L_FPRINTF(stream, "# Timings #\n");
    L_FPRINTF(stream, "###########\n");
    L_FPRINTF(stream, "\n");

    L_FPRINTF(stream, "mst_eval: %.2f  # ms / token during generation\n",
            1.0e-3 * ctx->t_eval_us / ctx->n_eval);
    L_FPRINTF(stream, "mst_p_eval: %.2f  # ms / token during prompt processing\n",
            1.0e-3 * ctx->t_p_eval_us / ctx->n_p_eval);
    L_FPRINTF(stream, "mst_sample: %.2f  # ms / token during sampling\n",
            1.0e-3 * ctx->t_sample_us / ctx->n_sample);
    L_FPRINTF(stream, "n_eval: %d  # number of tokens generated (excluding the first one)\n", ctx->n_eval);
    L_FPRINTF(stream, "n_p_eval: %d  # number of tokens processed in batches at the beginning\n", ctx->n_p_eval);
    L_FPRINTF(stream, "n_sample: %d  # number of sampled tokens\n", ctx->n_sample);
    L_FPRINTF(stream, "t_eval_us: %" PRId64 "  # total microseconds spent generating tokens\n", ctx->t_eval_us);
    L_FPRINTF(stream, "t_load_us: %" PRId64 "  # total microseconds spent loading the model\n", ctx->t_load_us);
    L_FPRINTF(stream, "t_p_eval_us: %" PRId64 "  # total microseconds spent prompt processing\n", ctx->t_p_eval_us);
    L_FPRINTF(stream, "t_sample_us: %" PRId64 "  # total microseconds spent sampling\n", ctx->t_sample_us);
    L_FPRINTF(stream, "ts_eval: %.2f  # tokens / second during generation\n",
            1.0e6 * ctx->n_eval / ctx->t_eval_us);
    L_FPRINTF(stream, "ts_p_eval: %.2f  # tokens / second during prompt processing\n",
            1.0e6 * ctx->n_p_eval / ctx->t_p_eval_us);
    L_FPRINTF(stream, "ts_sample: %.2f  # tokens / second during sampling\n",
