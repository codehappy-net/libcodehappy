...
void ggml_cuda_error(const char * stmt, const char * func, const char * file, int line, const char * msg) {
    ...
    L_FPRINTF(stderr, "CUDA error: %s\n", msg);
    L_FPRINTF(stderr, "  current device: %d, in function %s at %s:%d\n", id, func, file, line);
    L_FPRINTF(stderr, "  %s\n", stmt);
...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    if (err != cudaSuccess) {
        L_FPRINTF(stderr, "%s: failed to initialize " GGML_CUDA_NAME ": %s\n", __func__, cudaGetErrorString(err));
        return info;
    }
    ...
    for (int id = 0; id < info.device_count; ++id) {
        ...
        L_FPRINTF(stderr, "  Device %d: %s, compute capability %d.%d, VMM: %s\n", id, prop.name, prop.major, prop.minor, device_vmm ? "yes" : "no");
...
struct ggml_cuda_pool_leg : public ggml_cuda_pool {
...
#ifdef DEBUG_CUDA_MALLOC
        L_FPRINTF(stderr, "%s[%d]: %d buffers, max_size = %u MB, pool_size = %u MB, requested %u MB\n", __func__, device, nnz,
    ...
    void free(void * ptr, size_t size) override {
        ...
        L_FPRINTF(stderr, "WARNING: cuda buffer pool full, increase MAX_CUDA_BUFFERS\n");
        ggml_cuda_set_device(device);
...
GGML_CALL static ggml_backend_buffer_t ggml_backend_cuda_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (err != cudaSuccess) {
        L_FPRINTF(stderr, "%s: allocating %.2f MiB on device %d: cudaMalloc failed: %s\n", __func__, size/1024.0/1024.0, buft_ctx->device, cudaGetErrorString(err));
        return nullptr;
    }
...
static void * ggml_cuda_host_malloc(size_t size) {
    ...
    if (err != cudaSuccess) {
        // clear the error
        cudaGetLastError();
        L_FPRINTF(stderr, "%s: warning: failed to allocate %.2f MiB of pinned memory: %s\n", __func__,
    ...
    switch (dst->op) {
        ...
        case GGML_OP_MUL_MAT:
            if (dst->src[0]->ne[3] != dst->src[1]->ne[3]) {
                L_FPRINTF(stderr, "%s: cannot compute %s: src0->ne[3] = %" PRId64 ", src1->ne[3] = %" PRId64 " - fallback to CPU\n", __func__, dst->name, dst->src[0]->ne[3], dst->src[1]->ne[3]);
                return false;
            } else {
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: disabling CUDA graphs due to GPU architecture\n", __func__);
#endif
        }
    }
    ...
    if (use_cuda_graph) {
        ...
        for (int i = 0; i < cgraph->n_nodes; i++) {
...
#ifndef NDEBUG
                L_FPRINTF(stderr, "%s: disabling CUDA graphs due to split buffer\n", __func__);
#endif
            }
...
#ifndef NDEBUG
                L_FPRINTF(stderr, "%s: disabling CUDA graphs due to mul_mat_id\n", __func__);
#endif
            }
            ...
            if (node->op == GGML_OP_ADD && node->src[1] && node->src[1]->ne[1] > 1) {
                ...
                use_cuda_graph = false;
#ifndef NDEBUG
                L_FPRINTF(stderr, "%s: disabling CUDA graphs due to batch size > 1 [%s] [%ld %ld %ld %ld]\n", __func__, node->name, node->ne[0], node->ne[1], node->ne[2], node->ne[3]);
#endif
            }
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: disabling CUDA graphs due to too many consecutive updates\n", __func__);
#endif
        }
    }
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                if (!ok) {
                    L_FPRINTF(stderr, "%s: error: op not supported %s (%s)\n", __func__, node->name, ggml_op_name(node->op));
                }
                GGML_ASSERT(ok);
            }
        }
...
#ifndef NDEBUG
                L_FPRINTF(stderr, "%s: disabling CUDA graphs due to failed graph capture\n", __func__);
#endif
            } else {
...
#ifndef NDEBUG
            L_FPRINTF(stderr, "%s: CUDA graph update failed\n", __func__);
#endif
...
GGML_CALL ggml_backend_t ggml_backend_cuda_init(int device) {
    if (device < 0 || device >= ggml_backend_cuda_get_device_count()) {
        L_FPRINTF(stderr, "%s: error: invalid device %d\n", __func__, device);
        return nullptr;
    }

    ggml_backend_cuda_context * ctx = new ggml_backend_cuda_context(device);
    if (ctx == nullptr) {
        L_FPRINTF(stderr, "%s: error: failed to allocate context\n", __func__);
        return nullptr;
    }
...
GGML_CALL bool ggml_backend_cuda_register_host_buffer(void * buffer, size_t size) {
    ...
    if (err != cudaSuccess) {
        ...
        L_FPRINTF(stderr, "%s: warning: failed to register %.2f MiB of pinned memory: %s\n", __func__,
                size/1024.0/1024.0, cudaGetErrorString(err));
        return false;
    }
    return true;
