...
#define CUDA_CHECK(err)                                                                 \
    do {                                                                                \
        ...
        if (err_ != cudaSuccess) {                                                      \
            ...
            L_FPRINTF(stderr, "current device: %d\n", id);                                \
            exit(1);                                                                    \
        }                                                                               \
    } while (0)
...
#ifdef DEBUG_CUDA_MALLOC
    L_FPRINTF(stderr, "%s: %d buffers, max_size = %u MB, tot_size = %u MB, requested %u MB\n", __func__, nnz,
...
void ggml_init_cublas() {
    ...
    if (!initialized) {
        ...
        for (int64_t id = 0; id < g_device_count; ++id) {
            ...
            L_FPRINTF(stderr, "  Device %ld: %s, compute capability %d.%d\n", id, prop.name, prop.major, prop.minor);
...
void * ggml_cuda_host_malloc(size_t size) {
    ...
    if (err != cudaSuccess) {
        ...
        cudaGetLastError();
        L_FPRINTF(stderr, "WARNING: failed to allocate %.2f MB of pinned memory: %s\n",
...
void ggml_cuda_set_main_device(int main_device) {
    if (main_device >= g_device_count) {
        L_FPRINTF(stderr, "warning: cannot set main_device=%d because there are only %d devices. Using device %d instead.\n",
