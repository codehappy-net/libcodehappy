...
#ifndef NDEBUG
        L_FPRINTF(stderr, "%s: warning: slow copy from %s to %s\n", __func__, ggml_backend_buffer_name(src->buffer), ggml_backend_buffer_name(dst->buffer));
#endif
...
#ifndef NDEBUG
    L_FPRINTF(stderr, "%s: registered backend %s\n", __func__, name);
#endif
...
ggml_backend_t ggml_backend_reg_init_backend_from_str(const char * backend_str) {
    ...
    if (backend_i == SIZE_MAX) {
        L_FPRINTF(stderr, "%s: backend %s not found\n", __func__, backend_name);
        return NULL;
    }
...
GGML_CALL static ggml_backend_buffer_t ggml_backend_cpu_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (data == NULL) {
        L_FPRINTF(stderr, "%s: failed to allocate buffer of size %zu\n", __func__, size);
        return NULL;
    }
...
GGML_CALL static ggml_backend_buffer_t ggml_backend_cpu_hbm_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (result != 0) {
        L_FPRINTF(stderr, "failed to allocate HBM buffer of size %zu\n", size);
        return NULL;
    }
...
static int ggml_backend_sched_backend_from_buffer(ggml_backend_sched_t sched, const struct ggml_tensor * tensor) {
    ...
    L_FPRINTF(stderr, "%s: error: no backend supports buffer type %s used in tensor %s\n",
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        if (cur_split < sched->n_splits && i == sched->splits[cur_split].i_start) {
            ...
            for (int j = 0; j < sched->splits[cur_split].n_inputs; j++) {
                L_FPRINTF(stderr, "[%s (%5.5s)] ", sched->splits[cur_split].inputs[j]->name,
        ...
        L_FPRINTF(stderr, "node #%3d (%10.10s): %20.20s (%5.5s) [%5.5s %8.8s]:", i, ggml_op_name(node->op), node->name,
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            L_FPRINTF(stderr, " %20.20s (%5.5s) [%5.5s %8.8s]", src->name,
...
static void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    if (sched->ctx == NULL) {
        L_FPRINTF(stderr, "%s: failed to initialize context\n", __func__);
        GGML_ASSERT(false);
    }
...
#ifdef DEBUG_PASS1
    L_FPRINTF(stderr, "PASS 1 ASSIGNMENTS\n"); ggml_backend_sched_print_assignments(sched, graph);
#endif
...
#ifdef DEBUG_PASS2
    L_FPRINTF(stderr, "PASS 2 ASSIGNMENTS\n"); ggml_backend_sched_print_assignments(sched, graph);
#endif
...
#ifdef DEBUG_PASS3
    L_FPRINTF(stderr, "PASS 3 ASSIGNMENTS\n"); ggml_backend_sched_print_assignments(sched, graph);
#endif
...
#ifdef DEBUG_PASS4
    L_FPRINTF(stderr, "PASS 4 ASSIGNMENTS\n"); ggml_backend_sched_print_assignments(sched, graph);
#endif
...
#ifndef NDEBUG
        L_FPRINTF(stderr, "%s: failed to allocate graph, reserving\n", __func__);
#endif
        ggml_gallocr_reserve_n(sched->galloc, sched->graph, sched->node_backend_ids, sched->leaf_backend_ids);
        if (!ggml_gallocr_alloc_graph(sched->galloc, sched->graph)) {
            L_FPRINTF(stderr, "%s: failed to allocate graph\n", __func__);
            return false;
        }
    }
...
struct ggml_backend_graph_copy ggml_backend_graph_copy(ggml_backend_t backend, struct ggml_cgraph * graph) {
    ...
    if (ctx_allocated == NULL || ctx_unallocated == NULL) {
        L_FPRINTF(stderr, "failed to allocate context for graph copy\n");
        free(hash_set.keys);
        free(node_copies);
        free(node_init);
    ...
    if (buffer == NULL) {
        L_FPRINTF(stderr, "failed to allocate buffer for graph copy\n");
        free(hash_set.keys);
        free(node_copies);
        free(node_init);
        ggml_free(ctx_allocated);
