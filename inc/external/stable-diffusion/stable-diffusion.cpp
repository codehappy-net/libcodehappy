...
static SDLogLevel log_level = SDLogLevel::ERROR;
...
#define SD_LOG_DEBUG(format, ...) SD_LOG(SDLogLevel::DEBUG, format, ##__VA_ARGS__)
#define SD_LOG_INFO(format, ...) SD_LOG(SDLogLevel::INFO, format, ##__VA_ARGS__)
#define SD_LOG_WARN(format, ...) SD_LOG(SDLogLevel::WARN, format, ##__VA_ARGS__)
#define SD_LOG_ERROR(format, ...) SD_LOG(SDLogLevel::ERROR, format, ##__VA_ARGS__)
...
ggml_tensor* load_tensor_from_file(ggml_context* ctx, const std::string& file_path) {
    ...
    if (!file.is_open()) {
        SD_LOG_ERROR("failed to open '%s'", file_path.c_str());
        return NULL;
    }
    int32_t n_dims;
    int32_t length;
    ...
    if (file.eof()) {
        SD_LOG_ERROR("incomplete file '%s'", file_path.c_str());
        return NULL;
    }
...
class CLIPTokenizer {
   ...
   public:
    ...
    std::vector<int> encode(std::string text) {
        ...
        ss << "]";
        SD_LOG_DEBUG("split prompt \"%s\" to tokens %s", original_text.c_str(), ss.str().c_str());
        return bpe_tokens;
    }
};
...
struct FrozenCLIPEmbedderWithCustomWords {
    ...
    std::pair<std::vector<int>, std::vector<float>> tokenize(std::string text,
...

        {
            ...
            ss << "]";
            SD_LOG_DEBUG("parse '%s' to %s", text.c_str(), ss.str().c_str());
        }
...
class StableDiffusionGGML {
   public:
    ...
    bool load_from_file(const std::string& file_path, Schedule schedule) {
        SD_LOG_INFO("loading model from '%s'", file_path.c_str());
        ...
        SD_LOG_DEBUG("verifying magic");
        // verify magic
        {
            uint32_t magic;
            file.read(reinterpret_cast<char*>(&magic), sizeof(magic));
            if (magic != GGML_FILE_MAGIC) {
                SD_LOG_ERROR("invalid model file '%s' (bad magic)", file_path.c_str());
                return false;
            }
        }

        SD_LOG_DEBUG("loading hparams");
        // load hparams
        file.read(reinterpret_cast<char*>(&ftype), sizeof(ftype));

        int model_type = (ftype >> 16) & 0xFFFF;
        if (model_type >= MODEL_TYPE_COUNT) {
            SD_LOG_ERROR("invalid model file '%s' (bad model type value %d)", file_path.c_str(), ftype);
            return false;
        }
        SD_LOG_INFO("model type: %s", model_type_to_str[model_type]);
        ...
        SD_LOG_INFO("ftype: %s", ggml_type_name(wtype));
        if (wtype == GGML_TYPE_COUNT) {
            SD_LOG_ERROR("invalid model file '%s' (bad ftype value %d)", file_path.c_str(), ftype);
            return false;
        }

        SD_LOG_DEBUG("loading vocab");
        // load vocab
        {
            int32_t n_vocab = 0;
            ...
            if (n_vocab != cond_stage_model.text_model.vocab_size) {
                SD_LOG_ERROR("invalid model file '%s' (bad vocab size %d != %d)",
        ...
        SD_LOG_DEBUG("ggml tensor size = %d bytes", (int)sizeof(ggml_tensor));
        {
            ...
            SD_LOG_DEBUG("clip params ctx size = % 6.2f MB", ctx_size / (1024.0 * 1024.0));
...

        {
            ...
            SD_LOG_DEBUG("unet params ctx size = % 6.2f MB", ctx_size / (1024.0 * 1024.0));
...

        {
            ...
            SD_LOG_DEBUG("vae params ctx size = % 6.2f MB", ctx_size / (1024.0 * 1024.0));
        ...
        SD_LOG_DEBUG("preparing memory for the weights");
        ...
        float alphas_cumprod[TIMESTEPS];
        {
            ...
            while (true) {
                ...
                if (tensors.find(name.data()) != tensors.end()) {
                    ...
                    if (name.find("quant") == std::string::npos && name.find("first_stage_model.encoder.") == std::string::npos) {
                        SD_LOG_WARN("unknown tensor '%s' in model file", name.data());
                    } else {
                        if (!vae_decode_only) {
                            SD_LOG_WARN("unknown tensor '%s' in model file", name.data());
                            return false;
                ...
                if (tensor->type != ttype) {
                    SD_LOG_ERROR("tensor '%s' has wrong type in model file: got %s, expect %s",
            ...
            for (auto pair : tensors) {
                ...
                if (tensor_names_in_file.find(pair.first) == tensor_names_in_file.end()) {
                    SD_LOG_ERROR("tensor '%s' not in model file", pair.first.c_str());
                    some_tensor_not_init = true;
                }
            }
            if (tensor_names_in_file.find("alphas_cumprod") == tensor_names_in_file.end()) {
                SD_LOG_ERROR("tensor alphas_cumprod not in model file");
                some_tensor_not_init = true;
            }
            ...
            SD_LOG_DEBUG("model size = %.2fMB", total_size / 1024.0 / 1024.0);
        }
        ...
        SD_LOG_INFO("total params size = %.2fMB (clip %.2fMB, unet %.2fMB, vae %.2fMB)",
        ...
        SD_LOG_INFO("loading model from '%s' completed, taking %.2fs", file_path.c_str(), (t1 - t0) * 1.0f / 1000);
        file.close();
        ...
        if (is_using_v_parameterization) {
            ...
            SD_LOG_INFO("running in v-prediction mode");
        } else {
            SD_LOG_INFO("running in eps-prediction mode");
        }

        if (schedule != DEFAULT) {
            switch (schedule) {
                case DISCRETE:
                    SD_LOG_INFO("running with discrete schedule");
                    denoiser->schedule = std::make_shared<DiscreteSchedule>();
                    break;
                case KARRAS:
                    SD_LOG_INFO("running with Karras schedule");
                ...
                case DEFAULT:
                    ...
                    break;
                default:
                    SD_LOG_ERROR("Unknown schedule %i", schedule);
                    abort();
            }
        }
...
#endif
        int64_t t1 = ggml_time_ms();
        SD_LOG_INFO("check is_using_v_parameterization_for_sd2 completed, taking %.2fs", (t1 - t0) * 1.0f / 1000);
        SD_LOG_DEBUG("diffusion graph use %.2fMB runtime memory: static %.2fMB, dynamic %.2fMB",
                  (ctx_size + ggml_curr_max_dynamic_size()) * 1.0f / 1024 / 1024,
                  ctx_size * 1.0f / 1024 / 1024,
                  ggml_curr_max_dynamic_size() * 1.0f / 1024 / 1024);
        SD_LOG_DEBUG("%zu bytes of dynamic memory has not been released yet", ggml_dynamic_size());
    ...
    ggml_tensor* get_learned_condition(ggml_context* res_ctx, const std::string& text) {
        ...
        // calculate the amount of memory required
        {
            ...
            ctx_size += ggml_used_mem(ctx) + ggml_used_mem_of_data(ctx);
            SD_LOG_DEBUG("condition context need %.2fMB static memory, with work_size needing %.2fMB",
                      ctx_size * 1.0f / 1024 / 1024,
                      cplan.work_size * 1.0f / 1024 / 1024);
            ggml_free(ctx);
        }
        ...
        SD_LOG_DEBUG("building condition graph completed: %d nodes, %d leafs",
        ...
        int64_t t1 = ggml_time_ms();
        SD_LOG_DEBUG("computing condition graph completed, taking %.2fs", (t1 - t0) * 1.0f / 1000);
    ...
    ggml_tensor* sample(ggml_context* res_ctx,
        ...
        auto denoise = [&](ggml_tensor* input, float sigma, int step) {
            ...
            int64_t t1 = ggml_time_ms();
            if (step > 0) {
                SD_LOG_INFO("step %d sampling completed, taking %.2fs", step, (t1 - t0) * 1.0f / 1000);
                SD_LOG_DEBUG("diffusion graph use %.2fMB runtime memory: static %.2fMB, dynamic %.2fMB",
                          (ctx_size + ggml_curr_max_dynamic_size()) * 1.0f / 1024 / 1024,
                          ctx_size * 1.0f / 1024 / 1024,
                          ggml_curr_max_dynamic_size() * 1.0f / 1024 / 1024);
        ...
        switch (method) {
            case EULER_A: {
                SD_LOG_INFO("sampling using Euler A method");
                ggml_set_dynamic(ctx, false);
            ...
            {
                SD_LOG_INFO("sampling using Euler method");
                ggml_set_dynamic(ctx, false);
                struct ggml_tensor* d = ggml_dup_tensor(ctx, x);
                ggml_set_dynamic(ctx, params.dynamic);
            ...
            case HEUN: {
                SD_LOG_INFO("sampling using Heun method");
                ggml_set_dynamic(ctx, false);
                struct ggml_tensor* d = ggml_dup_tensor(ctx, x);
            ...
            case DPM2: {
                SD_LOG_INFO("sampling using DPM2 method");
                ggml_set_dynamic(ctx, false);
                struct ggml_tensor* d = ggml_dup_tensor(ctx, x);
                struct ggml_tensor* x2 = ggml_dup_tensor(ctx, x);
                ggml_set_dynamic(ctx, params.dynamic);
            ...
            case DPMPP2S_A: {
                SD_LOG_INFO("sampling using DPM++ (2s) a method");
                ggml_set_dynamic(ctx, false);
                struct ggml_tensor* noise = ggml_dup_tensor(ctx, x);
                struct ggml_tensor* d = ggml_dup_tensor(ctx, x);
                struct ggml_tensor* x2 = ggml_dup_tensor(ctx, x);
            ...
            {
                SD_LOG_INFO("sampling using DPM++ (2M) method");
                ggml_set_dynamic(ctx, false);
            ...
            {
                SD_LOG_INFO("sampling using modified DPM++ (2M) method");
                ggml_set_dynamic(ctx, false);
                struct ggml_tensor* old_denoised = ggml_dup_tensor(ctx, x);
                ggml_set_dynamic(ctx, params.dynamic);
            ...
            default:
                SD_LOG_ERROR("Attempting to sample with nonexisting sample method %i", method);
                abort();
        }
...
#endif
            SD_LOG_DEBUG("computing vae graph completed, taking %.2fs", (t1 - t0) * 1.0f / 1000);
...
std::vector<uint8_t> StableDiffusion::txt2img(const std::string& prompt,
    ...
    int64_t t1 = ggml_time_ms();
    SD_LOG_INFO("get_learned_condition completed, taking %.2fs", (t1 - t0) * 1.0f / 1000);
    ...
    SD_LOG_INFO("decode_first_stage completed, taking %.2fs", (t3 - t2) * 1.0f / 1000);
...
std::vector<uint8_t> StableDiffusion::img2img(const std::vector<uint8_t>& init_img_vec,
    ...
    SD_LOG_INFO("img2img %dx%d", width, height);

    std::vector<float> sigmas = sd->denoiser->schedule->get_sigmas(sample_steps);
    size_t t_enc = static_cast<size_t>(sample_steps * strength);
    SD_LOG_INFO("target t_enc is %zu steps", t_enc);
    ...
    int64_t t1 = ggml_time_ms();
    SD_LOG_INFO("encode_first_stage completed, taking %.2fs", (t1 - t0) * 1.0f / 1000);
    ...
    int64_t t2 = ggml_time_ms();
    SD_LOG_INFO("get_learned_condition completed, taking %.2fs", (t2 - t1) * 1.0f / 1000);
    if (sd->free_params_immediately) {
        sd->curr_params_mem_size -= ggml_used_mem(sd->clip_params_ctx);
        ggml_free(sd->clip_params_ctx);
        sd->clip_params_ctx = NULL;
    ...
    // print_ggml_tensor(x_0);
    int64_t t3 = ggml_time_ms();
    SD_LOG_INFO("sampling completed, taking %.2fs", (t3 - t2) * 1.0f / 1000);
    if (sd->free_params_immediately) {
        sd->curr_params_mem_size -= ggml_used_mem(sd->unet_params_ctx);
        ggml_free(sd->unet_params_ctx);
        sd->unet_params_ctx = NULL;
    ...
    SD_LOG_INFO("decode_first_stage completed, taking %.2fs", (t4 - t3) * 1.0f / 1000);
