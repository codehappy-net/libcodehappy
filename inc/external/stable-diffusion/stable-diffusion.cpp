...
sd_ctx_t* new_sd_ctx(const char* model_path_c_str,
    ...
    if (model_path_c_str != nullptr)
	    model_path = model_path_c_str;
    std::string vae_path;
    if (vae_path_c_str != nullptr)
    	vae_path = vae_path_c_str;
    std::string taesd_path;
    if (taesd_path_c_str != nullptr)
    	taesd_path = taesd_path_c_str;
    std::string control_net_path;
    if (control_net_path_c_str != nullptr)
    	control_net_path = control_net_path_c_str;
    std::string embd_path;
    if (embed_dir_c_str != nullptr)
    	embed_dir_c_str = embed_dir_c_str;
    std::string id_embd_path;
    if (id_embed_dir_c_str != nullptr)
    	id_embd_path = id_embed_dir_c_str;
    std::string lora_model_dir;
    if (lora_model_dir_c_str != nullptr)
    	lora_model_dir = lora_model_dir_c_str;
...
sd_image_t* txt2img(sd_ctx_t* sd_ctx,
    ...
    sd_image_t* result_images = generate_image(sd_ctx,
                                               work_ctx,
                                               NULL,
                                               (prompt_c_str == nullptr) ? "" : prompt_c_str,
                                               (negative_prompt_c_str == nullptr) ? "" : negative_prompt_c_str,
                                               clip_skip,
                                               cfg_scale,
                                               width,
                                               height,
